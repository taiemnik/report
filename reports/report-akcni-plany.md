# Report — Vyhodnocení akčních plánů 2023–2026

Hloubková analýza tří vyhodnocených akčních plánů (AP 2023–2024, 2024–2025, 2025–2026) a návrhu AP 2026–2027, vytěžená přímo z originálních vyhodnocovacích tabulek. Optika: **Value for Money** — co se za naplánované úsilí reálně dotáhlo do konce.

> Metodická poznámka: Stavy aktivit jsou klasifikovány ze sloupce „Vyhodnocení" v originálních XLSX. „splněno" = splněno/realizováno/dokončeno; „probíhá" = zahájeno/plněno průběžně; dále nezahájeno, pozastaveno, zrušeno a neuvedeno (prázdné hodnocení). Akční plány navazují na 4 prioritní oblasti Strategického plánu 2023–2038.

## Manažerské shrnutí

Město plánuje ambiciózně a většinu aktivit rozjede, ale **dokončí jen necelou třetinu** v rámci dané dvouleté periody. Napříč třemi vyhodnocenými AP je z **500 aktivit splněno 178 (36 %)**, dalších **240 (48 %) je rozpracovaných** a jen 38 (8 %) zůstalo nezahájených. Skutečně „mrtvých" aktivit je minimum — pozastaveno 9, zrušena 1.

Čtyři závěry, které z dat vystupují:

1. **Vysoká rozpracovanost, střední dokončenost.** Kombinace 36 % splněno + 48 % probíhá znamená, že úřad má rozjeto téměř vše, co si naplánoval — slabinou není start, ale dotažení a uzavírání aktivit v termínu periody.
2. **Oblast C (Vyvážený rozvoj) je motor i úzké hrdlo.** Nese 244 z 500 aktivit (49 %) a zároveň drží nejvíc nezahájených položek. Sem jde největší díl kapacity města a tady se také nejvíc „nestíhá".
3. **Oblast D (Sociální zázemí) zrychluje.** Dokončenost roste 47 % → 60 % → 71 % napříč třemi periodami — nejlepší a zlepšující se výsledek ze všech oblastí, byť na nejmenším objemu aktivit.
4. **Návrh AP 2026–2027 priority nemění.** Rozložení aktivit po oblastech zůstává prakticky stejné (C dominantní, 80 z 159). Není zde signál vědomé korekce směrem k podceněným tématům — pokračuje se v zaběhnuté struktuře.

## Plnění napříč obdobími

| Akční plán | Aktivit | Splněno | Probíhá | Nezahájeno | Pozast. | % splněno |
|---|---|---|---|---|---|---|
| AP 2023–2024 | 162 | 57 | 74 | 15 | 2 | 35 % |
| AP 2024–2025 | 166 | 58 | 84 | 11 | 3 | 35 % |
| AP 2025–2026 | 172 | 63 | 82 | 12 | 4 | 37 % |
| **3 AP celkem** | **500** | **178** | **240** | **38** | **9** | **36 %** |
| Návrh AP 2026–2027 | 159 | — | — | — | — | (plán) |

Míra dokončenosti je napříč obdobími pozoruhodně stabilní (35–37 %). To není výkyv jednoho roku, ale **strukturální rys** — město si dlouhodobě plánuje zhruba třikrát víc, než stihne v periodě uzavřít. Pro řízení je to zásadní informace: buď je plán předimenzovaný vůči kapacitě, nebo je dvouletý horizont pro typ aktivit příliš krátký.

## Plnění podle prioritních oblastí

Součet napříč třemi vyhodnocenými AP, podle oblasti Strategického plánu:

| Oblast | Aktivit (3 AP) | Podíl na úsilí | Charakter |
|---|---|---|---|
| **A — Ekonomika a podnikání** | 86 | 17 % | Stabilní ~50 % dokončenost, ale vysoký podíl nehodnocených položek |
| **B — Přitažlivost / image města a veřejný život** | 126 | 25 % | Nejnižší dokončenost (17–33 %), nejvíc rozpracovaného |
| **C — Vyvážený rozvoj** | 244 | 49 % | Největší objem, dokončenost 29–35 %, největší backlog nezahájených |
| **D — Sociální zázemí** | 44 | 9 % | Nejvyšší a rostoucí dokončenost (47 → 60 → 71 %) |

Dokončenost oblastí podle period:

| Oblast | AP 23–24 | AP 24–25 | AP 25–26 |
|---|---|---|---|
| A | 50 % | 50 % | 47 % |
| B | 27 % | 17 % | 33 % |
| C | 32 % | 35 % | 29 % |
| D | 47 % | 60 % | 71 % |

Čtení podle Value for Money: nejlépe „proměňuje plán ve výsledek" oblast D. Oblast B systematicky vykazuje nejnižší podíl dokončeného — buď jde o aktivity dlouhodobé povahy (kultura, oživení centra, akce), které „nikdy nekončí", nebo o oblast, kde se hůře definuje, co znamená „splněno". Oblast C je objemově rozhodující, takže i mírné zlepšení její dokončenosti má největší absolutní dopad.

## Co zůstává nedotažené

Nezahájených a pozastavených aktivit je celkově málo (47 z 500), ale koncentrují se v oblasti C (infrastruktura, doprava, veřejný prostor) — tedy tam, kde aktivity vyžadují projektovou přípravu, majetkové vypořádání a externí financování. To je vysvětlitelné, ale řízensky důležité: **úzké hrdlo není rozhodnutí „zda", ale kapacita na přípravu a realizaci.** Samostatný seznam „K řešení" v každém AP (regenerace sadu E. Beneše, veřejná prostranství, dětská hřiště) tento přípravný backlog potvrzuje.

## Návrh AP 2026–2027 — co napovídá o prioritách

Návrh obsahuje 159 aktivit s rozložením A 28 / B 39 / C 80 / D 12. Struktura prakticky kopíruje předchozí periody — oblast C zůstává dominantní (50 % aktivit). Vysoký podíl zatím nehodnocených položek (zejména v A a C) odpovídá tomu, že jde o plán před zahájením. **Signál:** priority se nepřeskupují; pokud má vedení záměr posílit některou podceněnou oblast (viz [[report-komunikacni-gap.md]]), v návrhu to zatím není čitelné a stojí za vědomé rozhodnutí.

---

## Pro vedení města

Plánujete zhruba třikrát víc, než stihnete v periodě dokončit, a tento poměr se nemění tři cykly po sobě. To je buď kapacitní strop (málo lidí na projektovou přípravu v oblasti C), nebo příliš krátký dvouletý horizont. Doporučení: u příštího AP vědomě rozhodnout mezi dvěma cestami — **zúžit plán na realisticky dotažitelný objem**, nebo **posílit přípravné kapacity** v oblasti C. Oblast D ukazuje, že vysoká dokončenost je dosažitelná; vyplatí se zjistit, čím se D liší (jasnější zadání, kratší realizace, menší závislost na externím financování) a přenést to jinam.

## Pro zastupitele

Z 500 naplánovaných aktivit je 36 % hotových a 48 % rozpracovaných — město tedy plán nenechává ležet, ale dokončuje pomaleji, než plánuje. Mrtvých aktivit (zrušeno/pozastaveno) je minimum, což svědčí o realistickém plánování bez velkých „odpisů". Při schvalování dalšího AP je legitimní ptát se, proč je dvouletá dokončenost stabilně kolem třetiny a zda další plán neslibuje opět víc, než lze v období uzavřít.

## Pro občany

Město si na každé dva roky stanoví stovky konkrétních úkolů — od oprav chodníků po kulturní akce. Z těch se během dvou let zhruba třetina dokončí a polovina rozběhne; jen malá část se nezačne vůbec. Nejrychleji se daří v sociální oblasti (péče, bydlení, rodiny). Nejvíc úkolů — a nejvíc těch, co se protáhnou — je ve „vyváženém rozvoji", tedy ve stavbách, dopravě a veřejném prostoru, kde příprava trvá roky.

## Pro úřad (interní)

Klíčová provozní metrika je poměr splněno : probíhá : nezahájeno = 36 : 48 : 8. „Probíhá" je zdaleka největší kategorie, takže hodnota se schovává v rozpracovanosti, kterou stávající vyhodnocení nezachycuje (zahájeno = 0–99 %). Doporučení: u dalšího vyhodnocení zavést hrubý odhad rozpracovanosti (např. 25/50/75 %) u položek „zahájeno", aby šlo odlišit aktivitu těsně před dokončením od té, co se sotva rozjela. Vysoký podíl prázdných hodnocení v oblastech A a C zkresluje statistiku — stojí za to doplnit i u položek, které „nemají co hlásit".

## Metodika a zdroje

- **Zdroj:** originální vyhodnocovací tabulky `data/zdroje/ostatni dokumenty/` — `Vyhodnocení AP2023-2024 k 31.12.2023.xlsx`, `Vyhodnocení AP2024-2025_k 31.12.2024.xlsx`, `Vyhodnocení AP2025-2026_k 31.12.2025.xlsx`, `Návrh_AP2026-2027__finál.xlsx`.
- **Zpracování:** každý list A–D, řádky s vyplněným názvem aktivity; stav klasifikován ze sloupce „Vyhodnocení".
- **Omezení:** kategorie „probíhá" (zahájeno) nerozlišuje míru rozpracovanosti. Prázdná hodnocení (7 %) jsou vedena samostatně, ne jako nesplněno.
- **Ověření:** čísla jsou nezávisle přepočtena skriptem nad originály; doporučuji při zásadních rozhodnutích zkontrolovat proti zdrojovým tabulkám.
